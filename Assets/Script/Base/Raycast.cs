...
public class Raycast
{
    ...
    {
        return Physics2D.Raycast(_origin.position, new Vector2(_dir.x * _horizontalDirection, _dir.y),
            _maxDistance, _targetLayer);
    }
    ...
    public void OnDrawGizmo(Transform origin)
    {
        // Rayがヒットした場合で色を変える。
        RaycastHit2D hit;
        if (hit = Physics2D.Raycast(origin.position, new Vector2(_dir.x * _horizontalDirection, _dir.y),
            _maxDistance, _targetLayer))
        {
            //衝突時のRayを画面に表示
            Debug.DrawRay(
